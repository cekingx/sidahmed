	...
	<tbody>
		...
		<tr class="order" data-status="{{$order->status}}" data-type="{{$order->digital}}">
			<td>
				@if($order->file)
				<a href="{{$order->file}}">
					<img src="{{$order->file}}" class="ui small image">
				</a>
				@else
					No preview.
				@endif
			...
			<td>
				{{$order->quantity}} {{$order->quantity > 1 ? '('.$order->quantity_id.'/'.$order->quantity.')' : ''}}
			</td>
			<td>
				@if(!empty($order->data))
			...
			<td>
				...
				<a target="_blank" class="ui fluid button" href="{{route('tracker::status', ['order' => ($order->etsy_transaction ? $order->etsy_transaction : $order->id)])}}">
					Order Status
				</a>
				@endif
			</td>
			<td>
				...
				<a href="javascript:void(0)" class="settings trigger" data-order="{{$order->id}}">
					<i class="ellipsis horizontal large icon" {{$order->etsy_transaction && !$order->etsy_verified ? 'disabled' : ''}}></i>
				</a>
			</td>
		</tr>
		@endforeach
