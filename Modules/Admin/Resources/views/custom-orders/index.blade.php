		...
		@foreach($customOrdersList as $orderInfo)
			<tr class="img-link-tr">
				...
				<td>
					...
					<a target="_black" href="{{route('cities::editor')}}?orderPoster={{$orderDetails['id']}}" >{{$orderDetails['title']}}</a><br>
					@endforeach
					@endif
				</td>
				<td><a orderId="{{$orderInfo['id']}}" href="{{route('admin::custom-order.mark-complete',$orderInfo['id'])}}" >Mark As Complete</a></td>

				<td><a href="{{route('admin::custom-order.delete',$orderInfo['id'])}}" onclick="if (confirm('Are you sure you want to delete selected order?')){return true;}else{event.stopPropagation(); event.preventDefault();};" title="Delete Order">Delete</a>
</td>
			</tr>
		@endforeach
	</tbody>
